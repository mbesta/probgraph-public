...
int64_t OrderedCount(const Graph &g, float treshold, int k_, float tau, std::string graphName, int threads) {
    ...
    for (NodeID u = 0; u < g.num_nodes(); ++u) {
        for (NodeID *vp = g.out_neigh(u).begin(); vp < hstart[u]; vp++) {
            ...
            int m = (*sets[u]).union_count(*sets[*vp]);
            if(n * 1.0 / m > tau){
                total += 1;
            }
    ...
    std::cout << "RRR JP-JC KMV JP-JC_KMV " << graphName << " " << threads << " " << g.num_nodes() << " " << g.num_edges() << " " << treshold << " " << k << " " << m << " " << tau << " " << pp_time << " " << alg_time << " " << pp_time + alg_time <<  " " << total << std::endl;
    ...
    std::cout << "SSS JP-JC KMV JP-JC_KMV " << graphName << " " << threads << " " << g.num_nodes() << " " << g.num_edges() << " " << treshold << " " << k << " " << m << " " << tau << " " << approx_str_size << " " << initial_csr_size << " " << approx_str_size + initial_csr_size << " " << total << std::endl;
